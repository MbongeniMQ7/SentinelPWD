    ...
    <AppShell>
      ...
      <div className="px-5 mt-5 space-y-4">
        <section className="relative bg-surface rounded-2xl p-5 shadow-sm border-l-4 border-warning">
          ...
          <button className="mt-5 inline-flex items-center gap-1 text-[12px] font-extrabold tracking-wider text-warning-foreground uppercase">
        ...
        <section className="bg-surface rounded-2xl shadow-sm overflow-hidden">
          <div className="bg-primary text-primary-foreground px-5 py-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button className="text-primary-foreground/80"><Filter className="h-4 w-4" /></button>
              <button className="text-primary-foreground/80"><Search className="h-4 w-4" /></button>
            </div>
          </div>
          ...
          <button className="w-full py-4 text-[12px] font-extrabold tracking-wider text-ink-soft uppercase">Load 8 More Requests</button>
        </section>
      </div>
    </AppShell>
  );
...
function PredictRow({ initials, bg, fatigue }: { initials: string; bg: string; fatigue: string }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <button className="px-4 h-9 rounded-lg bg-warning text-[10px] font-extrabold tracking-wider text-ink uppercase">Auto-Grant</button>
    </div>
  );
}
  ...
  return (
    <li className="px-5 py-4">
      ...
      <div className="mt-3 flex gap-3">
        <button className="flex-1 h-11 rounded-xl bg-muted text-[12px] font-extrabold tracking-wider text-ink uppercase">Reject</button>
        <button className="flex-1 h-11 rounded-xl bg-primary text-primary-foreground text-[12px] font-extrabold tracking-wider uppercase">Approve</button>
      </div>
    </li>
  );
}
