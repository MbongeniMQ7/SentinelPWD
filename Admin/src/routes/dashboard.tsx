...
import { Users, AlertOctagon, Zap, Clock, Download, RefreshCw } from "lucide-react";
    ...
    <AppShell>
      ...
      <div className="px-5 pt-4">
        ...
        <div className="mt-4 flex gap-3">
          <button className="flex-1 h-11 rounded-xl bg-muted text-ink text-[12px] font-extrabold tracking-wider flex items-center justify-center gap-2">
            <Download className="h-4 w-4" /> EXPORT CSV
          </button>
          <button className="flex-1 h-11 rounded-xl bg-primary text-primary-foreground text-[12px] font-extrabold tracking-wider flex items-center justify-center gap-2">
      ...
      <div className="px-5 mt-5 space-y-4">
        <StatCard label="TOTAL EMPLOYEES" value="124" delta="+2.4% vs last month" deltaColor="text-success" icon={<Users className="h-4 w-4 text-ink" />} iconBg="bg-info-soft" />
        <StatCard label="HIGH-RISK EMPLOYEES" value="8" pill={<StatusBadge variant="critical">Immediate Attention</StatusBadge>} icon={<AlertOctagon className="h-4 w-4 text-critical" />} iconBg="bg-critical/10" stripe />
        <StatCard label="ACTIVE ALERTS" value="3" delta={<span className="flex items-center gap-1 text-ink-soft text-[12px]"><Clock className="h-3.5 w-3.5" /> Last triggered 14m ago</span>} icon={<Zap className="h-4 w-4 text-warning-foreground" />} iconBg="bg-warning/30" />
        ...
        <section className="bg-surface rounded-2xl p-5 shadow-sm">
          ...
          <Link to="/workforce" className="mt-5 block text-center text-[13px] font-extrabold tracking-wider text-ink uppercase">
            View All Incidents
          </Link>
        </section>
        ...
        <section className="bg-surface rounded-2xl p-5 shadow-sm">
          ...
          <div className="mt-5 space-y-4">
            <RiskBar label="Low Risk" pct={92} count="(114)" color="bg-success" />
            <RiskBar label="Moderate Risk" pct={5} count="(6)" color="bg-warning" />
            <RiskBar label="High Risk" pct={3} count="(4)" color="bg-critical" />
          </div>
        ...
        <section className="bg-surface-muted rounded-2xl p-5 shadow-sm">
          ...
          <div className="mt-4 flex items-center gap-4">
            ...
            <Donut total={124} segments={[{ pct: 0.69, color: "var(--warning)" }, { pct: 0.31, color: "oklch(0.78 0.06 240)" }]} />
          </div>
        </section>

        <button className="fixed bottom-24 right-[calc(50%-200px)] sm:right-6 h-12 w-12 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center" aria-label="Support">
