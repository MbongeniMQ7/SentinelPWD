...
const Companies = () => {
  ...
  useEffect(() => {
    async function load() {
      ...
      // For each company, find the first MANAGER profile and their monitoring type
      const enriched: CompanyRow[] = await Promise.all(
        (companyRows ?? []).map(async (c) => {
          ...
          // Detect dominant monitoring type from employee_profiles
  ...
  const getMonitoringIcon = (type: string | null) => {
    ...
    if (type === "HYBRID") return Radio; // shows two icons in JSX
    return Eye;
  };
