...
type Section = "general" | "security" | "notifications" | "subscriptions";
    ...
    <AppShell>
      ...
      <div className="px-5 pt-3 pb-6 space-y-5">
        ...
        <div className="bg-card text-card-foreground rounded-2xl p-4 shadow-card">
          ...
          {([
            { Icon: Sliders, label: "General Config",   key: "general" as Section },
            { Icon: Shield,  label: "Security & Keys",  key: "security" as Section },
            { Icon: Bell,    label: "Notifications",    key: "notifications" as Section },
            { Icon: Receipt, label: "Subscriptions",    key: "subscriptions" as Section },
          ] as const).map(({ Icon, label, key }) => (
            <button
              key={label}
              onClick={() => {
                if (key === "subscriptions") {
                  navigate({ to: "/owner/subscriptions" });
                } else {
                  setSection(key);
                }
              }}
              className={`w-full flex items-center justify-between py-3 px-2 rounded-xl ${section === key ? "bg-card-muted" : ""}`}
            >
              <span className="flex items-center gap-3 text-primary font-semibold"><Icon className="h-5 w-5" />{label}</span>
              {section !== key && <ChevronRight className="h-4 w-4 text-muted-foreground" />}
            </button>
          ))}
        </div>
        ...
        {section === "security" && (
          <div>
            <h3 className="font-display font-bold text-xl flex items-center gap-2"><Shield className="h-5 w-5 text-gold" />Security &amp; Keys</h3>
            <div className="bg-card text-card-foreground rounded-2xl p-5 shadow-card mt-3 space-y-4">
              <div>
              ...
              <div>
                <div className="label-eyebrow">SUBSCRIPTION PLANS</div>
