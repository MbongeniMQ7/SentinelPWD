  ...
  return (
    <div className="app-shell px-5 py-6 pb-10">
      <header className="flex items-center gap-3 mb-6">
        <Link to="/profile" className="h-9 w-9 rounded-full flex items-center justify-center">
      ...
      <Section label="App Preferences">
        ...
        <Row icon={<Globe className="h-4 w-4" />} title="Language" sub="English (UK)" trailing={<ChevronRight className="h-4 w-4 text-muted-foreground" />} />
      </Section>
      ...
      <Section label="Account">
        <Row icon={<AtSign className="h-4 w-4" />} title="Update Email" sub="m.chen@sentinelai.com" trailing={<ChevronRight className="h-4 w-4 text-muted-foreground" />} />
        <Row icon={<RotateCcw className="h-4 w-4" />} title="Change Password" sub="Last updated 3 months ago" trailing={<ChevronRight className="h-4 w-4 text-muted-foreground" />} />
        <Row icon={<ShieldCheck className="h-4 w-4" />} title="Two-Factor Authentication" sub={<span className="text-gold-foreground font-semibold">Enabled</span>} trailing={<ChevronRight className="h-4 w-4 text-muted-foreground" />} />
      </Section>

      <Section label="Security">
        <Row icon={<Search className="h-4 w-4" />} title="Privacy Policy" trailing={<ExternalLink className="h-4 w-4 text-muted-foreground" />} />
        <Row icon={<FileCheck className="h-4 w-4" />} title="Terms of Service" trailing={<ExternalLink className="h-4 w-4 text-muted-foreground" />} />
      </Section>
      ...
      <div className="mt-6 text-center">
        <Link to="/support" className="text-xs font-bold text-gold-foreground tracking-wider">CONTACT SUPPORT</Link>
      </div>
    </div>
  );
}
...
function Row({ icon, iconBg = "bg-secondary", title, sub, trailing }: { icon: React.ReactNode; iconBg?: string; title: string; sub?: React.ReactNode; trailing?: React.ReactNode }) {
  return (
