      ...
      <div className="px-5 mt-5 space-y-5">
        <section className="bg-surface rounded-2xl p-5 shadow-sm">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <button
              onClick={() => toast("Plan management coming soon")}
              className="h-11 rounded-xl bg-primary text-primary-foreground text-[12px] font-extrabold tracking-wider uppercase"
            >
              Manage Plan
            </button>
            <button
              onClick={() => {
                if (confirm("Cancel your subscription?")) toast.success("Subscription cancellation requested");
              }}
              className="h-11 rounded-xl bg-muted text-ink text-[12px] font-extrabold tracking-wider uppercase"
            >
              Cancel Subscription
            </button>
          </div>
        ...
        <section className="bg-surface-muted rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => toast.success("Preparing all invoices for download…")}
              className="text-[11px] font-extrabold tracking-wider text-ink uppercase"
            >
              Download All
            </button>
          </div>
            ...
            {[
              ...
              <li key={inv.id} className="flex items-center gap-3 bg-surface rounded-xl p-3">
                ...
                <div className="text-right">
                  ...
                  <button
                    onClick={() => toast.success(`Downloading invoice #SAI-${inv.id}`)}
                    className="text-ink-soft inline-flex"
        ...
        <section className="relative rounded-2xl p-5 text-primary-foreground overflow-hidden bg-primary">
          ...
          <button
            onClick={() => toast("Pro Plus upgrade flow coming soon")}
            className="mt-5 w-full h-12 rounded-xl bg-warning text-ink text-[13px] font-extrabold tracking-wider uppercase"
          >
            Upgrade to Pro Plus
          </button>
        ...
        <section className="bg-surface-muted rounded-2xl p-5 shadow-sm">
          ...
          <div className="mt-3 flex items-center gap-3 bg-surface rounded-xl p-3">
            ...
            <button
              onClick={() => toast("Edit payment method — coming soon")}
          ...
          <button
            onClick={() => toast("Add new payment method — coming soon")}
            className="mt-3 w-full h-12 rounded-xl bg-surface border border-border text-[12px] font-extrabold tracking-wider text-ink uppercase flex items-center justify-center gap-2"
          >
