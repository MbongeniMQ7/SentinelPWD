...
function Alerts() {
  ...
  // Live fatigue alerts for the current worker, newest first, unacknowledged.
  const alertLog = useAlertLog();
  const liveAlerts = alertLog.filter(
    (a) => a.kind === "worker" && a.workerId === CURRENT_WORKER_ID && !a.acknowledged,
  );
  ...
  return (
    <div className="app-shell flex flex-col">
      ...
      <main className="flex-1 px-5 pb-6 space-y-5">
        ...
        {/* Live HIGH fatigue alerts — rendered with the same card style */}
        {liveHighAlerts.map((a) => (
          <div key={a.id} className="panel p-4 border-l-4 border-l-danger">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex justify-between gap-2">
                  ...
                  <span className="rounded-md bg-secondary px-2 py-1 text-[11px] font-semibold text-muted-foreground h-fit">{formatRelative(a.timestamp)}</span>
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  Your fatigue score reached {a.score}/100. {a.message}
                </p>
                <div className="mt-3 flex gap-2">
                  <Link to="/user/leave" className="rounded-xl bg-navy text-navy-foreground text-xs font-bold px-4 py-2.5">Request Break</Link>
                  <button onClick={() => { acknowledgeAlert(a.id); toast("Alert dismissed"); }} className="rounded-xl bg-secondary text-xs font-bold px-4 py-2.5">Dismiss</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Live MODERATE fatigue alerts — same gold-border card style */}
        {liveModerateAlerts.map((a) => (
          <div key={a.id} className="panel p-4 border-l-4 border-l-gold">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex justify-between gap-2">
                  <h3 className="font-display font-bold text-lg leading-tight">Elevated Fatigue<br />Risk</h3>
                  <span className="rounded-md bg-secondary px-2 py-1 text-[11px] font-semibold text-muted-foreground h-fit">{formatRelative(a.timestamp)}</span>
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  Your fatigue score reached {a.score}/100. {a.message}
        ...
        {/* Static fallback HIGH card — only shown when no live HIGH alerts exist */}
        {liveHighAlerts.length === 0 && !dismissed.includes("fatigue") && (
        <div className="panel p-4 border-l-4 border-l-danger">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 flex gap-2">
                <Link to="/user/leave" className="rounded-xl bg-navy text-navy-foreground text-xs font-bold px-4 py-2.5">Request Break</Link>
                <button onClick={() => dismiss("fatigue")} className="rounded-xl bg-secondary text-xs font-bold px-4 py-2.5">Dismiss</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <Link to="/user/leave" className="inline-block rounded-xl bg-navy text-navy-foreground text-xs font-bold px-4 py-2.5">
          Apply for rest
        </Link>

        <div className="flex items-center justify-between pt-2">
          <span className="label-eyebrow">Past 24 Hours</span>
          <button onClick={() => toast("Filter panel coming soon")} className="flex items-center gap-1 text-xs font-semibold text-muted-foreground">
        ...
        <button
          type="button"
          onClick={() => toast("Loading older alerts…")}
          className="w-full text-center label-eyebrow py-2"
        >
          Load Older Alerts
        </button>
