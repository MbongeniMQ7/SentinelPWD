          ...
          {companies.map(c => (
            <div key={c.id} className="bg-card text-card-foreground rounded-2xl p-5 shadow-card">
              ...
              <div className="border-t border-border mt-4 pt-3 flex items-center justify-between">
                ...
                <button onClick={() => nav("/company/aetheria")} className="text-primary text-sm font-semibold underline underline-offset-4 decoration-gold">View Details</button>
              </div>
            </div>
          ))}
        </div>

        <button onClick={() => nav("/companies/new")} className="fixed bottom-24 right-[calc(50%-13rem)] h-14 w-14 rounded-2xl bg-primary text-white flex items-center justify-center shadow-card">
